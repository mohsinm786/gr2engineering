                  ...
                  <ul className="block lg:flex lg:space-x-12 ml-auto">
                    {menuData.map((menuItem, index) => (
                      <li key={index} className="group relative">
                        <Link
                          href={menuItem.path || "#"} // Provide a default path like "#" for items without a specific path
                          onClick={!menuItem.path ? () => handleSubmenu(index) : undefined} // Trigger submenu only for items without a path
                          className={`flex cursor-pointer items-center justify-between py-2 text-base text-dark group-hover:text-primary dark:text-white/70 dark:group-hover:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${usePathName === menuItem.path
